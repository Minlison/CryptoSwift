...
public class EncryptedMessage: Message {
    ...
    /// Decrypts an encrypted message with a private SwiftyRSAKey and returns a clear message.
    ///
    /// - Parameters:
    ///   - SwiftyRSAKey: Private SwiftyRSAKey to decrypt the mssage with
    ///   - padding: Padding to use during the decryption
    /// - Returns: Clear message
    /// - Throws: SwiftyRSAError
    public func decrypted(with SwiftyRSAKey: PrivateKey, padding: SwiftyRSAPadding) throws -> ClearMessage {
        ...
        while idx < encryptedDataAsArray.count {
            ...
            let status = SecKeyDecrypt(SwiftyRSAKey.reference, padding, chunkData, idxEnd-idx, &decryptedDataBuffer, &decryptedDataLength)
