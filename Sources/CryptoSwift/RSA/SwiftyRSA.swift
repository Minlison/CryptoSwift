...
public typealias SwiftyRSAPadding = SecPadding
...
enum SwiftyRSA {
    ...
    static func data(forKeyReference reference: SecKey) throws -> Data {
        ...
        // On iOS 8/9, we need to add the SwiftyRSAKey again to the keychain with a temporary tag, grab the data,
        // and delete the SwiftyRSAKey again.
        } else {
    ...
    static func addKey(_ keyData: Data, isPublic: Bool, tag: String) throws ->  SecKey {
        ...
        if #available(iOS 10.0, *), #available(watchOS 3.0, *), #available(tvOS 10.0, *) {
            ...
            guard let SwiftyRSAKey = SecKeyCreateWithData(keyData as CFData, keyDict as CFDictionary, &error) else {
                throw SwiftyRSAError.keyCreateFailed(error: error?.takeRetainedValue())
            }
            return SwiftyRSAKey
            
        // On iOS 9 and earlier, add a persistent version of the SwiftyRSAKey to the system keychain
        } else {
            ...
            // Now fetch the SecKeyRef version of the SwiftyRSAKey
    ...
    /**
     This method strips the x509 header from a provided ASN.1 DER SwiftyRSAKey.
     If the SwiftyRSAKey doesn't contain a header, the DER data is returned as is.
     ...
     Example of headerless SwiftyRSAKey:
     ...
     Example of SwiftyRSAKey with X509 header (notice the additional ASN.1 sequence):
    ...
    static func stripKeyHeader(keyData: Data) throws -> Data {
        ...
        // Detect whether the sequence only has integers, in which case it's a headerless SwiftyRSAKey
